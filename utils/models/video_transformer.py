...
from models.transformer_module import Block, _cfg, PatchEmbed, get_sinusoid_encoding_table
...
class PretrainVisionTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.enc_dim = encoder_embed_dim
    ...
    def forward(self, x, mask):
        ...
        return x
    def decode(self, x):
        x_vis = self.encoder_to_decoder(x) # [B, N_vis, C_d]
        B, N, C = x_vis.shape
        # we don't unshuffle the correct visible token order, 
        # but shuffle the pos embedding accorddingly.
        expand_pos_embed = self.pos_embed.expand(B, -1, -1).type_as(x).to(x.device).clone().detach()
        pos_emd_vis = expand_pos_embed.reshape(B, -1, C)
        x_full = x_vis + pos_emd_vis[:,:x_vis.shape[1],:] # [B, N, C_d]
        x = self.decoder(x_full, 0) # [B, N_mask, 3 * 16 * 16]
        return x

@register_model
def pretrain_mae_small_patch16_224(img_size=256, pretrained=False, **kwargs):
...
@register_model
def pretrain_videomae_base_patch16_224(img_size=256, pretrained=False, init_ckpt = '', **kwargs):
    model = PretrainVisionTransformer(
        img_size=img_size,
        patch_size=16, 
        encoder_embed_dim=768, 
...
@register_model
def pretrain_videomae_large_patch16_224(img_size=256, pretrained=False, **kwargs):
    model = PretrainVisionTransformer(
        img_size=img_size,
        patch_size=16, 
